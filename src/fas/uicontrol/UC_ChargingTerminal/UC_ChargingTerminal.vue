<template>
<div class='UC_ChargingTerminal' v-show='mappingProps.visible' >
  <fasr_chargingPile_code
    :ref='el=>Widget["fasr_chargingPile"]=el'
    style='opacity:100%'
    class='fasr_chargingPile'
  instanceCode='fasr_chargingPile'
  :tabIndex='1'
  :localVars='LocalVars'    @OnFireEvent='fasr_chargingPile_OnFireEvent'
  >
  </fasr_chargingPile_code>
</div>
</template>
...
const fireUCEvent = defineEmits([ 'OnLoaded','closeCharge' ]);
const Widget =  {
  fasr_chargingPile : null,
  FireUCEvent : fireUCEvent,
};
const PageCode = ref('UC_ChargingTerminal');
const PageID = ref('eeba2ac1-a8e0-442b-abe0-04dcd571cecb');
const PageName = ref('充电中终端');
...
class CtrlMethods {
  refresh() {
    if (LocalVars.PileData) {
      ...
      LocalVars.pile.EstimatedFillingTime = '预计 ' + LocalVars.PileData.EstimatedFillingTime + ' 充满'; //预计充满时间 
      ...
      LocalVars.pile.PileSOC = LocalVars.PileData.PileSOC ? LocalVars.PileData.PileSOC + '%' : 0 + '%';//充电进度
...
async function fasr_chargingPile_OnFireEvent(e) {
  Widget.FireUCEvent('closeCharge',e);
}
...
const { fasr_chargingPile } = toRefs(reactive(Widget));
defineExpose({
  fasr_chargingPile,
