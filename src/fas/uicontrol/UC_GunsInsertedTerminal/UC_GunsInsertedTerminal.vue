<template>
  <div class='UC_GunsInsertedTerminal' v-show='mappingProps.visible'>
    <fasr_gunsInsertedPile_code :ref='el => Widget["fasr_gunsInsertedPile"] = el' style='opacity:100%'
      class='fasr_gunsInsertedPile' instanceCode='fasr_gunsInsertedPile' :tabIndex='1' :localVars='LocalVars'
      @OnFireEvent='fasr_gunsInsertedPile_OnFireEvent'>
    </fasr_gunsInsertedPile_code>
    <span class="float-link" @click="handleMsgClick">报文</span>
  </div>
</template>
...
const fireUCEvent = defineEmits([ 'OnLoaded', 'openCharge' ]);
const Widget = {
  fasr_gunsInsertedPile: null,
  FireUCEvent: fireUCEvent,
};
const PageCode = ref('UC_GunsInsertedTerminal');
const PageID = ref('52ac60f3-6c30-4ffb-8a41-f81e2f4dc92c');
const PageName = ref('已插枪终端');
...
const handleMsgClick = () => {
  ...
  const param = {
    StaID: LocalVars.PileData.StaID || '',
    StaName: LocalVars.PileData.StaName || '',
    CtrlID: '',
    CtrlName: '',
    PileName: LocalVars.PileData.PileName || '',
    PileID: LocalVars.PileData.PileID || ''
  };
  Funcs.OpenFasPage({
...
class CtrlMethods {
  refresh() {
    if (LocalVars.PileData) {
      ...
      LocalVars.pile.IsSetPileLimitSOC = LocalVars.PileData.IsSetPileLimitSOC;
      LocalVars.pile.PileLimitSOC = LocalVars.PileData.PileLimitSOC;

      LocalVars.pile.GunInsertionTimeDt = LocalVars.PileData.GunInsertionTimeDt ? LocalVars.PileData.GunInsertionTimeDt : '--';//已充时长
      ...
      LocalVars.pile.CustomerClassification = LocalVars.PileData.BillCode ? LocalVars.PileData.CustomerClassification : '';//私家车
      ...
      LocalVars.pile.ChargingEndTimeDt = LocalVars.PileData.ChargingEndTimeDt;
      if (LocalVars.PileData.BillCode) {
        ...
        LocalVars.pile.ChargingPower = '--';
        LocalVars.pile.ChargingMoney = '--';
      }

      LocalVars.pile.ChargingBeginTime = LocalVars.PileData.ChargingBeginTime ? LocalVars.PileData.ChargingBeginTime : '--';//充电开始时间
      LocalVars.pile.IsCharged = LocalVars.PileData.IsCharged;
...
async function Page_OnLoaded(e) {
  if (window.isPC()) {
    LocalVars.position = 'standard';
    LocalVars.maximized = false;
  }
  else {
    LocalVars.position = 'bottom';
    LocalVars.maximized = true;
  }
  CtrlMethod.refresh();
}
async function fasr_gunsInsertedPile_OnFireEvent(e) {
  Widget.FireUCEvent('openCharge', e);
}
...
const { fasr_gunsInsertedPile } = toRefs(reactive(Widget));
defineExpose({
  fasr_gunsInsertedPile,
